...
class PluginTaskmasterImplementationTask extends CommonDBTM {
    ...
    function showForm($id, array $options = []) {
        ...
        if (($this->fields['evidence_type'] ?? 0) == 1 && !empty($this->fields['evidence_data'] ?? '')) {
            echo "<div style='margin-bottom:10px;'><strong>Arquivo atual:</strong> <a href='".$CFG_GLPI['root_doc']."/plugins/taskmaster/front/download.php?id=".$this->fields['id']."&type=task' class='btn btn-sm btn-info' target='_blank'><i class='fas fa-download'></i> Baixar: ".($this->fields['evidence_data'] ?? '')."</a></div>";
        }
            ...
            if ($evidence_type == 2) { // Link
                ...
                if (isset($_FILES[$file_key]) && !empty($_FILES[$file_key]['name'])) {
                    if ($_FILES[$file_key]['error'] === UPLOAD_ERR_OK) {
                        ...
                        $dest_name = "task_" . $id . "_" . time() . "." . $extension;
